...
public class BTreeDB {
    ...
    */
            ...
            switch(input[0]){
                ...
                case "select":
                    if(input.length>2)
                        ...
                        select(par,Long.parseLong(input[1]),dBt,dVal,numRecords);
    ...
    /**
     * start from the root node and check all keys.. if you find the key itself then print it, 
     ...
     * @throws IOException 
     */
    public static long select(long root,long key,RandomAccessFile dBt,RandomAccessFile dVal,long numRecords)throws IOException{
        ...
        for(int i=0;i<4;i++){
            ...
            if(i==0 && !checkIfLast(root,dBt)){
                ...
                if(key<ikey && !isEmpty(location,dBt)){ //if the key being searched is less than the first key in the node, access the child to its left
                    location-=8;
                    return select(location,root,dBt,dVal,numRecords); //go to the child node and start scanning there
                }
     ...
     * @throws IOException 
     */
    public static void printSelect(long key, long offset, RandomAccessFile dVal)throws IOException{
        if(offset!=-1){
