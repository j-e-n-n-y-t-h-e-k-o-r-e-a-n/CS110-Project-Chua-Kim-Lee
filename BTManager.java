...
//    //check from top which way to go(left right) then check child.....
    ...
    public void insert(long parent,RandomAccessFile db,long key,long numRecords) throws IOException{
        //beginning
        if(nNodes==1){ //if only first node is there
            System.out.println(nNodes);
            if(checkNotFull(nNodes-1,db)){ //FIRST ONE SAVES
                ...
                nm.split(db,getAllNums(key,nNodes-1,db),nNodes-1, getAllOffsets(numRecords,nNodes-1,db));
                nNodes = nm.returnNodes();
            }
            //end of record
            sort(db,numRecords+1,nNodes-1);
